...
## 2.5 Lazada-搜索稳定性保障
背景：搜索业务是Lazada的核心业务，DAU 在千万级别左右，为了确保产品的稳定性，围绕请求成功率、空结果率、响应时间、白频率等指标，解决数个线上紧急问题
- 分钟级监控
- 白屏的检测8060原则等
- Crash订阅响应（Crash的订阅响应链路是怎样的？）
...
结合技术栈和业务方向
### 百度-地图
客户端iOS方向负责人，动态化负责人 & 步行、骑行导航方向负责人
客户端工作内容
- 为指标负责，包括偏航率等等
...
客户端iOS端侧负责人 & 联盟业务负责人
iOS端侧工作内容：
- 深度理解业务和架构的基础上，优化业务的基础架构，目标提效，降低错误率等
- 团队技术氛围
- 负责产品的稳定性
...
- 在行业的积累，不管是业务还是技术
 ...
 复杂问题，具象化，颗粒化，可量化。举一个实际的例子，比如说客诉的问题，无法被妥善处理。客诉的问题，为什么难处理？因为不是标准化的东西。
 再比如，如何降低与竞品的差距，在性能方面怎么做的？我们怎么衡量和竞品的差距？这是一个抽象的问题，但是需要具体的，可执行的方案？如何去做的

 更多的聚焦于结果，聚焦于线和面：比如说，平时做需求，往往只聚焦于这个需求。在Lazada这儿，受到+1的影响，考虑问题和场景需要更高的维度。其实，很多需求都不是一揽子买卖，很多需求都会迭代，即使需求停止了，但是这个需求的产物，也能支持各种形态。比如说联盟的任务系统，任务系统可以做的很简单，把很多复杂问题交给服务去处理。但是，任务其实是一个链路线条，我们设计了任务的实现方式，包括完成部分和全部完成这样，对任务每个环节做了抽象，设置了trigger机制，将判断放在客户端处理
